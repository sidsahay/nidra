...
AnimatedCharacterData::AnimatedCharacterData(const aiScene *scene, const char* vshaderPath, const char* fshaderPath, const char* texturePath) : scene(scene)
{
    ...
    {
        nodeNameToAnim[std::string(scene->mAnimations[0]->mChannels[i]->mNodeName.data)] = scene->mAnimations[0]->mChannels[i];
    }
    ...
    for (auto meshIdx = 0; meshIdx < numMeshes; meshIdx++)
    {
        ...
        for (auto i = 0; i < mesh->mNumBones; i++)
        {
            ...
            {
                boneNameToInfo[name] = {boneNumber, nodeNameToAnim[name], aiMatrix4x4ToGlm(mesh->mBones[i]->mOffsetMatrix)};
                boneNumber++;
...
void AnimatedCharacterData::_UpdateBonesRec(aiNode *node, glm::mat4 parentTransform)
{
    ...
    if (found != nodeNameToAnim.end())
    {
        ...
        auto boneTranslateGlm = glm::translate(glm::mat4(1.f), glm::vec3(boneTranslateVec.x, boneTranslateVec.y, boneTranslateVec.z));
